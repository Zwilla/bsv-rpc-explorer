...
configPaths.filter(fs.existsSync).forEach(path => {
    ...
    console.log('BSVEXP_BITCOIND_USER=', process.env.BSVEXP_BITCOIND_USER);
...
if (process.env.BSVEXP_BASIC_AUTH_PASSWORD) {
    app.disable('x-powered-by');
    app.use(auth(process.env.BSVEXP_BASIC_AUTH_PASSWORD));
}
            ...
            for (var key in mempoolInfo) {
                try {
                    if (mempoolMapping[key]) {
                        points.push({
                            measurement: `${global.coinConfig.name.toLowerCase()}.mempool.${mempoolMapping[key]}`,
                            fields: {value: mempoolInfo[key]}
                        });
                    }
                } catch (err) {
            ...
            for (var key in miningInfo) {
                try {
                    if (miningMapping[key]) {
                        points.push({
                            measurement: `${global.coinConfig.name.toLowerCase()}.mining.${miningMapping[key].name}`,
                            fields: {value: miningMapping[key].transform(miningInfo[key])}
                        });
                    }
                } catch (err) {
            ...
            for (var key in blockchainInfo) {
                try {
                    if (blockchainMapping[key]) {
                        points.push({
                            measurement: `${global.coinConfig.name.toLowerCase()}.blockchain.${blockchainMapping[key]}`,
                            fields: {value: blockchainInfo[key]}
                        });
                    }
                } catch (err) {
            ...
            coreApi.getBlocksByHeight(blockHeights).then(function (blocks) {
                ...
                for (var i = 0; i < blocks.length; i++) {
                    ...
                    for (var key in blockInfo) {
                        points.push({
                            measurement: `${global.coinConfig.name.toLowerCase()}.blocks.${key}`,
                            fields: {value: blockInfo[key]},
                            timestamp: timestamp
                        });
                    }
...
app.runOnStartup = function () {
    ...
    coreApi.getNetworkInfo().then(function (getnetworkinfo) {
        ...
        if (global.influxdb != null) {
            logNetworkStats();
            setInterval(logNetworkStats, 60000);
    ...
    if (config.electrumXServers && config.electrumXServers.length > 0) {
        electrumApi.connectToServers().then(function () {
            ...
            console.log("Error 31207_ask_zwilla: " + err + ", while initializing ElectrumX API");
        });
    }
